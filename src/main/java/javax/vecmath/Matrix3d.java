...
Cloneable {
    ...
    public final void setRow(int row, double x2, double y2, double z) {
        switch (row) {
            case 0: {
                this.m00 = x2;
                this.m01 = y2;
    ...
    public final void setColumn(int column, double x2, double y2, double z) {
        switch (column) {
            case 0: {
                this.m00 = x2;
                this.m10 = y2;
    ...
    public final void set(AxisAngle4d a1) {
        ...
        if (mag < 1.110223024E-16) {
            ...
            double xy = ax * ay;
            double yz = ay * az;
            this.m00 = t2 * ax * ax + cosTheta;
            this.m01 = t2 * xy - sinTheta * az;
            this.m02 = t2 * xz + sinTheta * ay;
            this.m10 = t2 * xy + sinTheta * az;
            this.m11 = t2 * ay * ay + cosTheta;
            this.m12 = t2 * yz - sinTheta * ax;
            this.m20 = t2 * xz - sinTheta * ay;
            this.m21 = t2 * yz + sinTheta * ax;
            this.m22 = t2 * az * az + cosTheta;
        }
    }
    ...
    public final void transform(Tuple3d t2) {
        ...
        double z = this.m20 * t2.x + this.m21 * t2.y + this.m22 * t2.z;
        t2.set(x2, y2, z);
    }
    ...
    static void compute_svd(double[] m, double[] outScale, double[] outRot) {
        ...
        if (m[3] * m[3] < 1.110223024E-16) {
            ...
            g2 = 1.0 / Math.sqrt(m[0] * m[0] + m[3] * m[3]);
            double c1 = m[0] * g2;
            double s1 = m[3] * g2;
            tmp[0] = c1 * m[0] + s1 * m[3];
            tmp[1] = c1 * m[1] + s1 * m[4];
        ...
        if (!(m[6] * m[6] < 1.110223024E-16)) {
            if (m[0] * m[0] < 1.110223024E-16) {
                ...
                g2 = 1.0 / Math.sqrt(m[0] * m[0] + m[6] * m[6]);
                double c2 = m[0] * g2;
                double s2 = m[6] * g2;
                tmp[0] = c2 * m[0] + s2 * m[6];
                tmp[1] = c2 * m[1] + s2 * m[7];
        ...
        if (m[2] * m[2] < 1.110223024E-16) {
            ...
            g2 = 1.0 / Math.sqrt(m[1] * m[1] + m[2] * m[2]);
            double c3 = m[1] * g2;
            double s3 = m[2] * g2;
            tmp[1] = c3 * m[1] + s3 * m[2];
            m[2] = -s3 * m[1] + c3 * m[2];
        ...
        if (!(m[7] * m[7] < 1.110223024E-16)) {
            if (m[4] * m[4] < 1.110223024E-16) {
                ...
                g2 = 1.0 / Math.sqrt(m[4] * m[4] + m[7] * m[7]);
                double c4 = m[4] * g2;
                double s4 = m[7] * g2;
                tmp[3] = c4 * m[3] + s4 * m[6];
                m[6] = -s4 * m[3] + c4 * m[6];
    ...
    static int compute_qr(double[] s2, double[] e, double[] u2, double[] v) {
        double vtemp;
        double utemp;
        ...
        for (int k2 = 0; k2 < 10 && !converged; ++k2) {
            double shift = Matrix3d.compute_shift(s2[1], e[1], s2[2]);
            double f = (Math.abs(s2[0]) - shift) * (Matrix3d.d_sign(c_b48, s2[0]) + shift / s2[0]);
            double g2 = e[0];
            double r2 = Matrix3d.compute_rot(f, g2, sinr, cosr, 0, first);
            f = cosr[0] * s2[0] + sinr[0] * e[0];
            e[0] = cosr[0] * e[0] - sinr[0] * s2[0];
            g2 = sinr[0] * s2[1];
            s2[1] = cosr[0] * s2[1];
            r2 = Matrix3d.compute_rot(f, g2, sinl, cosl, 0, first);
            first = 0;
            s2[0] = r2;
            f = cosl[0] * e[0] + sinl[0] * s2[1];
            s2[1] = cosl[0] * s2[1] - sinl[0] * e[0];
            g2 = sinl[0] * e[1];
            e[1] = cosl[0] * e[1];
            e[0] = r2 = Matrix3d.compute_rot(f, g2, sinr, cosr, 1, first);
            f = cosr[1] * s2[1] + sinr[1] * e[1];
            e[1] = cosr[1] * e[1] - sinr[1] * s2[1];
            g2 = sinr[1] * s2[2];
            s2[2] = cosr[1] * s2[2];
            s2[1] = r2 = Matrix3d.compute_rot(f, g2, sinl, cosl, 1, first);
            f = cosl[1] * e[1] + sinl[1] * s2[2];
            s2[2] = cosl[1] * s2[2] - sinl[1] * e[1];
            e[1] = f;
            utemp = u2[0];
        ...
        if (Math.abs(e[1]) < 4.89E-15) {
            Matrix3d.compute_2X2(s2[0], e[0], s2[1], s2, sinl, cosl, sinr, cosr, 0);
            utemp = u2[0];
            u2[0] = cosl[0] * utemp + sinl[0] * u2[3];
            u2[3] = -sinl[0] * utemp + cosl[0] * u2[3];
            utemp = u2[1];
            ...
            Matrix3d.compute_2X2(s2[1], e[1], s2[2], s2, sinl, cosl, sinr, cosr, 1);
            utemp = u2[3];
        ...
        if ((ga = Math.abs(gt = g2)) == 0.0) {
            ...
            if (gasmal) {
                ...
                if (gasmal) {
                    ...
                    ssmin = ha / a;
                    ssmax = fa * a;
                    t2 = mm == 0.0 ? (l2 == 0.0 ? Matrix3d.d_sign(c_b3, ft) * Matrix3d.d_sign(c_b4, gt) : gt / Matrix3d.d_sign(d2, ft) + m / t2) : (m / (s2 + t2) + m / (r2 + l2)) * (a + 1.0);
    ...
    static double compute_rot(double f, double g2, double[] sin, double[] cos, int index, int first) {
        double r2;
        double sn;
        double cs;
    ...
    private static final boolean almostEqual(double a, double b2) {
        double absB;
        double max;
        if (a == b2) {
            return true;
